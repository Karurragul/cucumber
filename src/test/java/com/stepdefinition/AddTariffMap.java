...
public class AddTariffMap  {
	...
	public void the_user_is_filling_all_the_datas(DataTable Custdetails) {
		...
		Base.driver.findElement(By.id("rental1")).sendKeys(custdeta.get(1).get("monthy rental"));

		Base.driver.findElement(By.id("local_minutes")).sendKeys(custdeta.get(2).get("Free loc min"));

		Base.driver.findElement(By.id("inter_minutes")).sendKeys(custdeta.get(0).get("Free Intern min"));

		Base.driver.findElement(By.id("sms_pack")).sendKeys(custdeta.get(3).get("Free sms"));

		Base.driver.findElement(By.id("minutes_charges")).sendKeys(custdeta.get(3).get("local per min"));

		Base.driver.findElement(By.id("inter_charges")).sendKeys(custdeta.get(2).get("Intern per min"));

		Base.driver.findElement(By.id("sms_charges")).sendKeys(custdeta.get(1).get("sms per charges"));
